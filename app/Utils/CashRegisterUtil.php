...
class CashRegisterUtil extends Util
{
    ...
    public function getRegisterTransactionDetails($user_id, $open_time, $close_time)
    {
        $product_details = Transaction::where('transactions.created_by', $user_id)
            ...
            ->join('purchase_lines AS PSL', 'P.id', '=', 'PSL.product_id')
            ->leftjoin('brands AS B', 'P.brand_id', '=', 'B.id')
            ->groupBy('B.id')
            ->select(
                'B.name as brand_name','transactions.final_total','P.name as product_name','TSL.unit_price_inc_tax as unit_price','P.id as product_id','PSL.purchase_price_inc_tax as purchase_price',
                DB::raw('SUM(TSL.quantity) as total_quantity'),
                DB::raw('SUM(transactions.final_total*TSL.quantity) as total_amount')
            )
