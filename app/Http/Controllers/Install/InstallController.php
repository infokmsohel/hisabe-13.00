...
class InstallController extends Controller
{
    ...
    public function postDetails(Request $request)
    {
        ...
        try {
            ...
            $input = $request->only(['APP_NAME', 'APP_TITLE', 'DB_HOST', 'DB_PORT', 'DB_DATABASE', 'DB_USERNAME', 'DB_PASSWORD',
                'MAIL_DRIVER','MAIL_FROM_ADDRESS', 'MAIL_FROM_NAME', 'MAIL_HOST', 'MAIL_PORT', 'MAIL_ENCRYPTION',
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            // This field Must Have to be removed on  2.13 version's update 
            //$this->CreateDefaultBusinessAccount();
            //$this->CreateDefaultAccountForContacts();
     ...
     protected function CreateDefaultBusinessAccount() {
        $businessdetails = \App\BusinessLocation::all();
        foreach($businessdetails as $details){
            $business_id = $details->business_id;
            $location_id = $details->id;
            $account=Account::create([
                'business_id'=>$business_id,
                'name'=>'Cash_'.$details->name,
                'account_number'=>$business_id.'-'.$location_id.'-'.'1001',
                'account_type'=>'saving_current',
                'track_no'=>$business_id.$location_id,
                'track_type'=>'own',
                'note'=>'Default Business Account',
    ...
    public function CreateDefaultAccountForContacts() {
        $allContacts = \App\Contact::all();
        foreach($allContacts as $contacts){
                
                $account=New Account();
                
                $account->business_id = $contacts->business_id;
                $account->name = $contacts->name;
                if ($contacts->type == 'customer'){
                    $account->account_number= $contacts->business_id.'-'.'2001';
                    $account->track_type='customer';
                }elseif ($contacts->type == 'supplier'){
                    $account->account_number= $contacts->business_id.'-'.'3001';
                    $account->track_type='supplier';
                     
                }elseif ($contacts->type == 'both'){
                    $account->account_number=$contacts->business_id.'-'.'4001';
                    $account->track_type='both';
                }
                $account->account_type='saving_current';
                // Have to make change
                
                $account->track_no = $contacts->id.$contacts->business_id;
                

                $account->note='welcome';
                $account->created_by = $contacts->created_by;
                $account->save();
