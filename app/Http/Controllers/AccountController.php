...
class AccountController extends Controller
{
        ...
        if (request()->ajax()) {
            $accounts = Account::leftjoin('account_transactions as AT', function ($join) {
                ...
                $join->whereNull('AT.deleted_at');
                                ...
                                ->select(['name', 'account_number', 'accounts.note', 'accounts.id','track_type',
    ...
    public function create()
    {
        ...
        $new_account_types = Account::newAccountTypes();
        $accountCategories = Account::accountCategory();
        $bankList = Bank::pluck('bank_name','id');

        //        $bankList = Account::bankList();
//        $bankList = \Illuminate\Support\Facades\DB::table('banks')->get();

        $divisions = \Illuminate\Support\Facades\DB::table("divisions")->pluck("bn_name","id");


        $mobileBank=Account::mobileBanking();

        return view('account.create')
            ->with(compact('account_types','new_account_types','accountCategories','bankList','mobileBank','divisions'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (request()->ajax()) {
            try {
                $input = $request->only(['name', 'account_number','note','account_category',
                    'new_account_type','bank_id','new_account_number','district_id','division_id','mobile_bank',

                    'number','card_name','card_number'
                ]);
                ...
                $input['new_account_type'] = $request->input('new_account_type');
                $input['account_category'] = $request->input('account_category');
                $input['bank_id'] = $request->input('bank_id');
                $input['new_account_number'] = $request->input('new_account_number');
                $input['division_id'] = $request->input('division_id');
                $input['district_id'] = $request->input('district_id');
                $input['mobile_bank'] = $request->input('mobile_bank');
                $input['number'] = $request->input('number');
                $input['card_name'] = $request->input('card_name');
                $input['card_number'] = $request->input('card_number');
    ...
    public function getDistrictList(Request $request)
    {
        ...
        $option ='<option selected value=""> Select District</option>';
        foreach($districts as $menu)
        {
            $option =$option.'<option value="'.$menu->id.'">'.$menu->bn_name.'</option>';
        }
