            ...
            <div class="box-body">
                @can('direct_sell.access')
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped ajax_view" id="new_sell_table">
                            <thead>
                            <tr>
                                <th>@lang('messages.date')</th>
                                <th>@lang('sale.invoice_no')</th>
                                <th>@lang('sale.customer_name')</th>
                                <th>@lang('sale.mobile')</th>
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready( function(){
            ...
            new_sell_table = $('#new_sell_table').DataTable({
                processing: true,
                serverSide: true,
                aaSorting: [[0, 'desc']],
                "ajax": {
                ...
                "fnDrawCallback": function (oSettings) {
                    ...
                    $('#footer_sale_total').text(sum_table_col($('#new_sell_table'), 'final-total'));

                    $('#footer_total_paid').text(sum_table_col($('#new_sell_table'), 'total-paid'));

                    $('#footer_total_remaining').text(sum_table_col($('#new_sell_table'), 'payment_due'));

                    $('#footer_total_sell_return_due').text(sum_table_col($('#new_sell_table'), 'sell_return_due'));

                    $('#footer_payment_status_count').html(__sum_status_html($('#new_sell_table'), 'payment-status-label'));

                    __currency_convert_recursively($('#new_sell_table'));
                },
