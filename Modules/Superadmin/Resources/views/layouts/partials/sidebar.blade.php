@can('superadmin')
	<li class="bg-red treeview {{ in_array($request->segment(1), ['superadmin']) ? 'active active-sub' : '' }}">
	    ...
	    <ul class="treeview-menu">
			...
			<li class="{{ $request->segment(2) == 'bank_list' ? 'active active-sub' : '' }}">
				<a href="{{action('\Modules\Superadmin\Http\Controllers\BankListController@index')}}">
					<i class="fa fa-envelope"></i>
					<span class="title">
						@lang('superadmin::lang.bank_list')
					</span>
				</a>
			</li>

			<li class="{{ $request->segment(2) == 'bank_list' ? 'active active-sub' : '' }}">
				<a href="{{action('\Modules\Superadmin\Http\Controllers\BusinessTypeController@index')}}">
					<i class="fa fa-envelope"></i>
					<span class="title">
						@lang('superadmin::lang.business_type')
					</span>
				</a>
			</li>
