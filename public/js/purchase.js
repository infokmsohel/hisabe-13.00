$(document).ready(function() {
    ...
    $('#supplier_id').change(function(){
        var suppier_id = $('#supplier_id').val();
        $.ajax({
            url: '/purchases/suppliers/account-id',
            method:'GET',
            data:{
                sid: suppier_id
            },
    ...
    $('input[name=serial]').change(function(){
        var id = $('input[name=serial]:checked' ).val();
        if(id === "inputSerial"){
            $('#inputSerial').removeClass('hidden');
            $('#generateSerial').addClass('hidden');
            $('#after-generate-serial').addClass('hidden');
        }else{
            $('#inputSerial').addClass('hidden');
            $('#generateSerial').removeClass('hidden');
        }        
    });
    ...
    $('#serialNumber').keypress(function(e){
        if(e.which === 13){ 
            var newtext = $('#serialNumber').val();            
            var x = newtext.split("\n");
            var oldx = oldValue.split("\n");
            var newSerial = x[x.length-1];
            if(jQuery.inArray(newSerial,oldx) >= 0 ){
                //console.log('value found');
                $('#serialNumber').val(oldValue);
            }else{
                //console.log('value Not found');  
                $.ajax({
                    url:'/purchase/check-serial',
                    method:'GET',
                    data:{
                    ...
                    success:function (status) {
                        if(status === 1){                            
                            $('.purchase_quantity').val(x.length);
                            $('.purchase_quantity').keyup();
                            oldValue =  $('#serialNumber').val();
                        }else{
                            $('#serialExists').modal('show');
                            $('#serialNumber').val(oldValue);
                        }
                    }
                });                
    ...
    var productname;
    $('#generateSerialIndex').click(function(){
        var prefix = $('#prefix').val();        
        var startSerial = parseInt($('#startSerial').val());
        var endSerial = parseInt($('#endSerial').val());
        var list = '';
        var genSerial = '';
        $.ajax({
            url:'/purchase/get-product-info',
            method:'GET',
            data:{
                    id : productId
                },
                ...
                success:function (productname) {
                        for(i = startSerial; i <= endSerial; i++){
                            list += productname +' '+ prefix+'-'+i+'<br>';  
                            genSerial += prefix+'-'+i+'<br>'; 
                        }        
                        $('#after-generate-serial').removeClass('hidden');
                        $('#dyn-generateSerial').val(genSerial);
                        $('#after-generate-serial').html(list);
                        //update quaantity
                        $('.purchase_quantity').val(endSerial - startSerial +1);
                        $('.purchase_quantity').keyup();
    ...
    changePurchase = $(document).on('change', '.purchase_unit_cost_without_discount', function() {
